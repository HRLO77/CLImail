...
class User:
    ...
    def __eq__(self, other):
        ...
        )  # dunno why I added this function

    def __init__(
        self,
        password: typing.AnyStr,
        user: typing.AnyStr,
        smtp_server: typing.AnyStr = "smtp.gmail.com",
        imap_server: typing.AnyStr = "imap.gmail.com",
        smtp_port: typing.SupportsInt = 465,
        imap_port: typing.SupportsInt = 993,
    ):
        """
    ...
    def sendmail(
        self,
        reciever: typing.AnyStr,
        content: typing.AnyStr = "None",
        subject: typing.AnyStr = "None",
        cc: typing.List[typing.AnyStr] = None,
        attachments: typing.List[typing.AnyStr] = None,
    ):
        """
        ...
        if not attachments is None:
            ...
            for attachment in attachments:  # add the attachments
                part = MIMEApplication(
                    attachment.read(), Name=os.path.basename(attachment.name)
                )
                part[
                    "Content-Disposition"
                ] = 'attachment; filename="%s"' % os.path.basename(attachment.name)
                msg.attach(part)
        text = msg.as_string()
    ...
    def search(
        self,
        string: typing.AnyStr = None,
        requirements: typing.AnyStr = "(UNSEEN)",
        size: typing.SupportsInt = 10,
    ):
        """
        Looks for mail with the string provided and requirements as a tuple of bytes.
        """
        return tuple(
            self.imap_server.search(string, requirements)[1][0]
            .split()[-1 : 0 - (size + 1) : -1]
            .__reversed__()
        )

    def subscribe(
        self, mailbox: typing.AnyStr
    ):  # and don't forget to hit that like button and click the notificaion bell for more!
        """
    ...
    def mail_from_id(
        self, id: typing.Union[typing.ByteString, typing.AnyStr]
    ...
    def mail_from_ids(
        self, ids: typing.Iterable[typing.Union[typing.ByteString, typing.AnyStr]]
    ) -> typing.Generator:
        """
    ...
    def mail_from_template(self, message: email.message.Message):
        ...
        for n in message.get_payload():
            if isinstance(n, str):
                continue
            if n.get_content_type().startswith(
                "application"
            ) or n.get_content_type().startswith("image"):
                string += f"{n.get_filename()}\n"
        string += "\n================== End of Mail ======================\n"
        return string

    def save_attachments(
        self, message: email.message.Message, path: typing.AnyStr = r"\tmp"
    ) -> typing.Generator:
        """
    ...
    def contacts(self, size: typing.SupportsInt = 10):
        ...
        for i in mails:
            if "To" in i:
                for b in i["To"].split(","):
                    contacts.append(
                        b.removesuffix(">").removeprefix("<")
                        if not " " in b
                        else b.rsplit(" ")[-1].removesuffix(">").removeprefix("<")
                    )
        c = Counter(contacts)
    ...
    def reconnect(self):
        ...
        self.imap_server.login(self.email, self.password), self.smtp_server.login(
    ...
    def copy_mails(
        self,
        ids: typing.Iterable[typing.AnyStr or typing.ByteString],
        folder: typing.AnyStr,
    ):
